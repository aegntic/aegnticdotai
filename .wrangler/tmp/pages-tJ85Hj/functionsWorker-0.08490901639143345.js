...
var onRequestPost = /* @__PURE__ */ __name(async ({ request, env }) => {
  try {
    ...
    ).bind(name || "", email, subject || "General Inquiry", message, Math.floor(Date.now() / 1e3)).run();
...
var onRequestPost2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    ).bind(email, source || "website", "active", Math.floor(Date.now() / 1e3)).run();
...
// ../.wrangler/tmp/pages-tJ85Hj/functionsRoutes-0.9532324568695842.mjs
var routes = [
  {
